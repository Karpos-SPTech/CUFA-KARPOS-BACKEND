...
public class UsuarioController implements UsuarioControllerDoc {
  ...
  public ResponseEntity<List<UsuarioResponseDto>> listarUsuarios() {
    ...
    if (usuariosEncontrados.isEmpty()) { return ResponseEntity.status(204).build(); }
  ...
  public void atualizarUsuario(@PathVariable Long id, @RequestBody UsuarioData usuarioAtualizado, Principal principal) {
    ...
    if (!id.equals(idDoToken)) {
      throw new AccessDeniedException("Você não tem permissão para atualizar o perfil de outro usuário.");
    }
  ...
  public ResponseEntity<String> uploadCurriculoUsuario(
    ...
    try {
      ...
      return switch (e) {
        case IllegalArgumentException erro -> ResponseEntity.badRequest().body(erro.getMessage());

        case IOException erro -> ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("Erro ao processar o arquivo: " + erro.getMessage());

        case RuntimeException erro -> ResponseEntity.status(HttpStatus.NOT_FOUND).body(erro.getMessage());

        default -> ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Erro inesperado: " + e.getMessage());
      };
    }
  }
  ...
  @SecurityRequirement(name = "Bearer")
  public ResponseEntity<String> deletarCurriculoUsuario(@PathVariable Long userId) {
    try {
      ...
      if (nomeArquivoAntigo != null) { curriculoController.deletarArquivoFisico(nomeArquivoAntigo); }
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body("Erro ao deletar currículo: " + e.getMessage());
    }
  }
}
